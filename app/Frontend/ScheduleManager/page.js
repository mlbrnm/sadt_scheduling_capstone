...
export default function ScheduleManager() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem(
        ...
        JSON.stringify(submittedSchedulesData)
      );
    }
  }, [submittedSchedulesData]);
  ...
  const sortData = (data, option) => {
    if (!option) return data;
      ...
      switch (option) {
        ...
        case "timeslots":
          return [...data].sort((a, b) =>
            a.timeslots.localeCompare(b.timeslots)
          );
        default:
  ...
  const validateFile = (file) => {
    ...
    return (
      validTypes.includes(file.type) || file.name.match(/\.(xlsx|csv|pdf)$/i)
    );
  };
  ...
  const handleTimeslotUpload = async (e) => {
    ...
    try {
      ...
      if (!validateFile(file)) {
        throw new Error(
          "Invalid file type. Please upload an Excel, CSV, or PDF file."
        );
      }
  ...
  return (
    ...
    <div className="p-8">
      {/* Create New Schedule Button/Filter/Sort & Search */}
      <div className="flex flex-row items-center justify-center gap-10 pb-8">
        <button
          className="button-primary text-white px-6 py-4 rounded"
          onClick={() => router.push("/Frontend/ScheduleManager/NewSchedule")}
        >
          Create New Schedule
        </button>
        ...
        <div className="text-lg text-bold flex items-center gap-1">
          ...
          <select
            ...
            defaultValue="Date Submitted, newest"
            value={sortOption}
          >
            ...
            <option value="timeslots" className="text-primary">
              Timeslots uploaded
            </option>
          </select>
        </div>
          ...
          <table className="table-auto min-w-full">
            ...
            <tbody>
              {displayedData.map((schedule, index) => (
                <tr
                  key={index}
                  className="grid grid-cols-6 gap-8 py-3 border-b pl-6"
                >
                  <td className="mt-1 self-center">
                    {formatDate(schedule.dateSubmitted)}
                  </td>
                  <td className="mt-1 self-center">{schedule.title}</td>
                  <td className="p-4-md mt-1.5 text-justify-center self-center">
                    <span
                      className={getProgramTypeColour(schedule.programType)}
                    >
                  ...
                  <td className="mt-1 self-center">
                    <button
                      className={`${getTimeslotsColour(schedule.timeslots)}`}
                      onClick={() => handleTimeslotUploadClick(index)}
                    >
                      {schedule.timeslots.toLowerCase().includes("uploaded") ? (
                        <div>
                          ...
                          <span className="ml-2 italic opacity-70 text-xs underline">
                            {" "}
                            (Click to upload new)
                          </span>
                        </div>
                  ...
                  <td className="flex justify-left mx-12 self-center">
                    <button className="button-primary text-white px-4 py-1.5 rounded hover:button-hover active:button-clicked">
                      View
                    </button>
                  </td>
                </tr>
