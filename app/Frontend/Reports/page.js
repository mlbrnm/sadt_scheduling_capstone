...
export default function Reports() {
  ...
  const generateProgramReportDonutChart = async (avgNewStudents, totalApplied, overallAggregateAdmitRate) => {
    return new Promise((resolve) => {
      ...
      const chart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            data: [avgNewStudents, totalApplied],
            backgroundColor: ['#C8E6C9', '#2E7D32'],
            borderWidth: 0
          }]
        },
        options: {
        ...
        plugins: [{
          ...
          beforeDraw: (chart) => {
            ...
            const subtext1 = `with ${avgNewStudents} students admitted`;
  ...
  // GENERATE PROGRAM REPORT CSV version
  ...
  // Setup page header for all PDF reports depending on report type
  ...
  const generateProgramReportPDF = async () => {
    ...
    const chartAreaStartY = yPos; // splitting top half of page into 2 columns for purposes of donut chart placement (LHS + RHS)

    // LHS: Add donut pie chart to page
    doc.addImage(donutChartImage, 'PNG', 25, chartAreaStartY, 70, 70);

    // RHS: Add associated semesters to page
    let rhsOfPage = chartAreaStartY;
    doc.setFontSize(10);
    doc.setFont("helvetica", "italic");
    doc.text('Associated Semesters', 115, rhsOfPage);
    rhsOfPage += 5;
    doc.text('(with available data):', 115, rhsOfPage);
    rhsOfPage += 9;
    ...
    semesterData.forEach((semester) => {
      doc.text(`- ${semester.semester}`, 115, rhsOfPage);
      rhsOfPage += 5;
    });
    yPos = chartAreaStartY + 80;
